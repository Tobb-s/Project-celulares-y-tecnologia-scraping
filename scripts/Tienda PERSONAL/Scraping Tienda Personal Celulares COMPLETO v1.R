...
# • Click “Tienda” (intenta y sigue si falla)
try({
...
repeat {
  ...
  # 6.1 Capturar tarjetas de producto
  Sys.sleep(2)
  ...
  for (i in seq_along(cards)) {
    ...
    # Volver al listado (breadcrumb “Celulares”)
    remote_driver$findElement("xpath", "//a[@href='/celulares' and normalize-space()='Celulares']")$clickElement()
    Sys.sleep(2)
    
    # Re‑capturar tarjetas tras el refresh
    cards <- remote_driver$findElements("css selector", "a[data-testid='product-card-container']")
  }
  
  # 6.2 Botón “Siguiente” y corte del loop
  siguiente <- remote_driver$findElements("xpath", "//a[contains(.,'Siguiente') and not(@disabled)]")
  if (length(siguiente) == 0 || page_ix >= CFG$max_pages) {
    message("✅ Fin de paginación.")
    break
  }
  siguiente[[1]]$clickElement()
  page_ix <- page_ix + 1L
  Sys.sleep(3)
}
