...
CFG <- list(
  ...
  spec_map = tribble(
    ...
    "Cámara principal",   "Cámara trasera (principal)",
...
# Scroll hasta el contenedor de solapas
...
# Construir vector de XPaths: uno por rótulo HTML
spec_xpath <- setNames(
...
# (Opcional) imprimir selectores para verificar
message("---- SELECTORES USADOS ----")
walk2(names(spec_xpath), spec_xpath,
      ~message(.x, " → ", .y))

# Capturar valores crudos
...
# Normalizar valores numéricos donde corresponda
...
# 8. EXTRAER PRECIOS (SOLAPA “LEGALES”) ----------------------------------------
tabs[[3]]$clickElement()
Sys.sleep(2)
